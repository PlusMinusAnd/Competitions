...
# Set2set.py 파일 상단
import sys
r = int(sys.argv[1]) if len(sys.argv) > 1 else 42
...
class GATSet2Set(nn.Module):
    def __init__(self, in_node_feats, hidden_size=128, num_heads=4, dropout=0.2):
        super().__init__()
        self.gat1 = GATConv(in_node_feats, hidden_size, num_heads)
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_graphs)):
        ...
        model = GATSet2Set(in_node_feats=in_node_feats, hidden_size=128, num_heads=4, dropout=0.2).to(device)
...
for a in alphas:
    ...
    if a > 1: continue
    c = 1 - a
    blended = a * oof_preds["lgbm"] + c * oof_preds["cat"]
...
a, c = best_weights
print(f"✅ 최적 가중치: LGBM={a:.2f}, CAT={c:.2f} | Score: {best_score:.5f}")

final_oof = a * oof_preds["lgbm"] + c * oof_preds["cat"]
final_test = a * test_preds["lgbm"] + c * test_preds["cat"]
...
def print_scores(y_true, y_pred, label=""):
    ...
    print(f"RMSE     : {rmse:.5f}", flush=True)
    print(f"NRMSE    : {nrmse:.5f}", flush=True)
    print(f"Pearson  : {pearson:.5f}", flush=True)
    print(f"Score📈  : {score:.5f}", flush=True)
    return score
...
# 🎯 기준 점수 설정
SCORE_THRESHOLD = 0.62463  # 원하는 기준값으로 설정
