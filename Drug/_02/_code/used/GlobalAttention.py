...
class GATNetImproved(nn.Module):
    def __init__(self, in_node_feats, hidden_size=128, num_heads=4, dropout=0.2):
        ...
        self.att_pool = GlobalAttentionPooling(
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_graphs)):
        ...
        model = GATNetImproved(in_node_feats=in_node_feats, hidden_size=128, num_heads=4, dropout=0.2).to(device)
