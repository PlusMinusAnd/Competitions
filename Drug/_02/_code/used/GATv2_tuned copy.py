...
class GATv2Set2Set(torch.nn.Module):
...
def collate_graphs(batch):
    if isinstance(batch[0], tuple):  # (graph, label)
...
def mc_dropout_predict(model, test_loader, T=100, device="cpu"):
    model.train()
    preds = []
    with torch.no_grad():
        for _ in range(T):
...
test_graphs = joblib.load("./Drug/_02/full_pipeline/0_dataset/test_graphs.pkl")
test_dataset = GraphDataset(test_graphs)
test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False, collate_fn=collate_graphs)
...
for fold in range(5):
    in_feats = test_graphs[0].ndata['h'].shape[1]
    model = GATv2Set2Set(in_node_feats=in_feats).to(device)
    model.load_state_dict(torch.load(f"./Drug/_02/full_pipeline/gat_fold{fold}.pt", map_location=device))
    mc_preds = mc_dropout_predict(model, test_loader, T=100, device=device)
    mc_all_folds.append(mc_preds)

# 통계 계산
mc_all_folds = np.stack(mc_all_folds, axis=0)
ensemble_mean = mc_all_folds.mean(axis=(0, 1))
ensemble_std = mc_all_folds.std(axis=(0, 1))
ci_lower = np.percentile(mc_all_folds, 2.5, axis=(0, 1))
ci_upper = np.percentile(mc_all_folds, 97.5, axis=(0, 1))

# 예시 시각화 (0번째 샘플)
idx = 0
example_preds = mc_all_folds[:, :, idx].flatten()

plt.hist(example_preds, bins=30, color='skyblue', edgecolor='black', alpha=0.7)
plt.rcParams['font.family'] = 'DejaVu Sans'
plt.axvline(ensemble_mean[idx], color='blue', linestyle='--', label=f'Mean: {ensemble_mean[idx]:.0f}')
plt.axvline(ci_lower[idx], color='red', linestyle='--', label=f'2.5%: {ci_lower[idx]:.0f}')
plt.axvline(ci_upper[idx], color='green', linestyle='--', label=f'97.5%: {ci_upper[idx]:.0f}')
plt.title("MC Dropout Prediction Distribution (Ensemble of Folds)")
...
# 🎯 기준 점수 설정
SCORE_THRESHOLD = 0.5  # 원하는 기준값으로 설정
