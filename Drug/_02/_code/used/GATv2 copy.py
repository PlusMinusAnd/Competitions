...
def atom_features(atom):
    ...
    features.append(COVALENT_RADII.get(atomic_num, 1.0))          # 공유 반지름 (직접 정의된 dict)
    features.append(ELECTRONEGATIVITY.get(atomic_num, 0.0))       # 전기음성도
    features.append(ptable.GetNOuterElecs(atomic_num))            # 최외각 전자 수
...
train_df = pd.read_csv(train_path)
test_df = pd.read_csv(test_path)
train_df['mol'] = train_df['Canonical_Smiles'].apply(smiles_to_mol_3d)
test_df['mol'] = test_df['Canonical_Smiles'].apply(smiles_to_mol_3d)
train_df['is_valid'] = train_df['mol'].notnull().astype(int)
test_df['is_valid'] = test_df['mol'].notnull().astype(int)

# 예외 저장
train_df[train_df['is_valid'] == 0].to_csv(f"{save_dir}/invalid_train_mol.csv", index=False)
test_df[test_df['is_valid'] == 0].to_csv(f"{save_dir}/invalid_test_mol.csv", index=False)
...
# 🔍 NaN-only 열 제거
train_desc_df = train_desc_df.dropna(axis=1, how='all')
test_desc_df = test_desc_df.dropna(axis=1, how='all')

# 🔍 Inhibition 제외하고 concat
