...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_graphs)):
        ...
        for epoch in range(1, 101):
            ...
            if val_rmse < best_rmse:
                best_rmse = val_rmse
                patience_counter = 0
                torch.save(model.state_dict(), f"./Drug/_02/full_pipeline/dmpnn_fold{fold}.pt")
            else:
                patience_counter += 1
        ...
        # OOF 저장
        model.load_state_dict(torch.load(f"./Drug/_02/full_pipeline/dmpnn_fold{fold}.pt"))
        model.eval()
        val_preds = []
        with torch.no_grad():
            for batch in val_loader:
    ...
    np.save("./Drug/_02/full_pipeline/pre_dmpnn_oof.npy", oof)
    np.save("./Drug/_02/full_pipeline/pre_dmpnn_preds.npy", test_preds)
...
dmpnn_oof = np.load("./Drug/_02/full_pipeline/pre_dmpnn_oof.npy")
boost_oof = np.load("./Drug/_02/full_pipeline/boost_oof.npy")
dmpnn_preds = np.load("./Drug/_02/full_pipeline/pre_dmpnn_preds.npy")
