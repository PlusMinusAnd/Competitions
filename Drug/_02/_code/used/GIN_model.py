...
class GINNet(nn.Module):
    def __init__(self, in_node_feats, hidden_size=128, dropout=0.2):
        super(GINNet, self).__init__()
        self.conv1 = GINConv(nn.Sequential(
            nn.Linear(in_node_feats, hidden_size),
            nn.ReLU(),
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_graphs)):
        ...
        model = GINNet(in_node_feats=in_node_feats, hidden_size=128, dropout=0.2).to(device)
        ...
        for epoch in range(1, 101):
            ...
            if val_rmse < best_rmse:
                best_rmse = val_rmse
                patience_counter = 0
                torch.save(model.state_dict(), f"./Drug/_02/full_pipeline/gin_fold{fold}.pt")
            else:
                patience_counter += 1
        ...
        model.load_state_dict(torch.load(f"./Drug/_02/full_pipeline/gin_fold{fold}.pt"))
        model.eval()
        val_preds = []
        with torch.no_grad():
            for batch in val_loader:
    ...
    np.save("./Drug/_02/full_pipeline/gin_oof.npy", oof)
    np.save("./Drug/_02/full_pipeline/gin_preds.npy", test_preds)
    ...
    filename = f"pre_GIN_{r}_({now}).csv"
    save_path = f"./Drug/_02/full_pipeline/{filename}"
    
    submission.to_csv(save_path, index=False)
    print("✅ gin_oof.npy, gin_preds.npy, pre_GIN.csv 저장 완료")
...
# ===== File: 04_01_stack_boost_earlystopping.py ===== #
...
dmpnn_oof = np.load("./Drug/_02/full_pipeline/gin_oof.npy")
boost_oof = np.load("./Drug/_02/full_pipeline/boost_oof.npy")
dmpnn_preds = np.load("./Drug/_02/full_pipeline/gin_preds.npy")
...
# 0.6752238391
# 📊 Final Ensemble α=0.05 결과
# RMSE     : 23.46451
# NRMSE    : 0.23611
# Pearson  : 0.46025
# Score📈  : 0.61207
# ✅ 최종 앙상블 파일 저장 완료 → submission_238_final_ensemble(20250706_2126).csv
# random_state : 238