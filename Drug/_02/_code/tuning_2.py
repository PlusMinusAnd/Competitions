...
seed_file = "./Drug/_02/full_pipeline/tuning_2.json"
...
# 6. 최종 저장
final_train = pd.DataFrame(X_train_pca, columns=[f'PC_{i}' for i in range(1, 1007)])
final_test = pd.DataFrame(X_test_pca, columns=[f'PC_{i}' for i in range(1, 1007)])
...
# # 🎯 기준 점수 설정
# SCORE_THRESHOLD = 0.64237  # 원하는 기준값으로 설정
...
# 📊 Final Ensemble (GAT+Boost+PyCaret), α=0.20
# RMSE     : 23.39285
# NRMSE    : 0.23538
# Pearson  : 0.51644
# Score📈  : 0.64053
...
# 📊 Final Ensemble (GAT+Boost+PyCaret), α=0.30
# RMSE     : 23.41947
# NRMSE    : 0.23565
# Pearson  : 0.51978
# Score📈  : 0.64207
...
# ===== File: Final Ensemble with Residual.py ===== #
def print_scores(y_true, y_pred, label=""):
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    
    # y_true의 최대값과 최소값이 같은 경우, NRMSE 계산 시 0으로 나누기 오류가 발생할 수 있으므로 분기 처리
    max_min_diff = np.max(y_true) - np.min(y_true)
    if max_min_diff == 0:
        nrmse = 0.0
    else:
        nrmse = rmse / max_min_diff
        
    # pearsonr 반환값이 NaN일 경우를 처리
    try:
        pearson = pearsonr(y_true, y_pred)[0]
        if np.isnan(pearson):
            pearson = 0.0  # NaN이면 0으로 처리
    except ValueError: # scipy.stats.pearsonr에서 발생하는 오류 처리
        pearson = 0.0
...
# 잔차(Residual) 계산
# 주력 모델: boost
residual_oof_gat = gat_oof - boost_oof
residual_oof_pycaret = pycaret_oof - boost_oof

# test 데이터에 대한 잔차 예측
residual_preds_gat = gat_preds - boost_preds
residual_preds_pycaret = pycaret_preds - boost_preds

# 최적 가중치 탐색 (잔차 모델에 대한 가중치)
print("🔍 Residual Blending 최적 α, β 탐색")
best_score = -np.inf
best_alpha_beta = (0.5, 0.5)
weights_space = np.linspace(0, 1, 11)

for alpha in weights_space:
    for beta in weights_space:
        if alpha + beta > 1.5: continue # 너무 큰 가중치는 제외
        final_oof_residual = boost_oof + alpha * residual_oof_gat + beta * residual_oof_pycaret
        score = print_scores(y, final_oof_residual, label=f"α(GAT_res)={alpha:.2f}, β(PyCaret_res)={beta:.2f}")
        if score > best_score:
            best_score = score
            best_alpha_beta = (alpha, beta)

alpha, beta = best_alpha_beta
print(f"✅ 최적 가중치: α(GAT_res)={alpha:.2f}, β(PyCaret_res)={beta:.2f} | Score: {best_score:.5f}")

# 최종 예측
final_oof = boost_oof + alpha * residual_oof_gat + beta * residual_oof_pycaret
final_preds = boost_preds + alpha * residual_preds_gat + beta * residual_preds_pycaret

# 점수 출력
final_score = print_scores(y, final_oof, label=f"Final Residual Ensemble (boost + {alpha:.2f}*GAT_res + {beta:.2f}*PyCaret_res)")
...
now = datetime.datetime.now().strftime("%Y%m%d_%H%M")
filename = f"submission_{r}_residual_ensemble({now}).csv"
submission.to_csv(f"./Drug/_02/full_pipeline/{filename}", index=False)
print(f"랜덤 시드값 : {r}")
print(f"✅ 최종 앙상블 파일 저장 완료 → {filename}")

# 기존 코드의 잔차 평가 로직 (optional)
# if final_score < SCORE_THRESHOLD:
#     shutil.rmtree("./Drug/_02/full_pipeline")
#     print(f"🚫 Score {final_score:.5f} < 기준 {SCORE_THRESHOLD} → 전체 디렉토리 삭제 완료")
# else:
