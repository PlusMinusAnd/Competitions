...
class GATGRUNet(nn.Module):
    ...
    def forward(self, g):
        with g.local_scope():
            ...
            h = F.elu(h)
            h = self.gat2(g, h).mean(1)  # shape = [total_nodes, hidden]
            
            # 📌 그래프별 평균 벡터 만들기
            g.ndata['h_final'] = h
            hg = dgl.mean_nodes(g, 'h_final')  # shape = [batch_size, hidden]

            h_gru_out, _ = self.gru(hg.unsqueeze(1))  # [B, 1, hidden]
            h_final = h_gru_out.squeeze(1)            # [B, hidden]

            return self.readout(h_final).squeeze(-1)
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_graphs)):
        ...
        model = GATGRUNet(in_node_feats=in_node_feats, hidden_size=128, num_heads=4, dropout=0.2).to(device)
...
# 📊 Final Ensemble α=0.00 결과
# RMSE     : 23.44927
# NRMSE    : 0.23595
# Pearson  : 0.46004
# Score📈  : 0.61205
# ✅ 최종 앙상블 파일 저장 완료 → submission_238_final_ensemble(20250706_2201).csv
# random_state : 238